...
async function main() {
  ...
  console.log(
    `Revoking authorized minter on ${hre.network.name} for token ${TOKEN_ADDRESS}...`,
  );

  const tx = await token.revokeAuthorizedMinter();
  console.log('Tx sent:', tx.hash);
  await tx.wait();

  console.log('Authorized minter revoked.');
}
