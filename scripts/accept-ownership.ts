...
async function main() {
  const privateKey = process.env.PENDING_OWNER_PRIVATE_KEY;

  if (!privateKey) {
    throw new Error('PENDING_OWNER_PRIVATE_KEY is not set in .env');
  }
  ...
  console.log(
    `Accepting ownership of Xirium on ${hre.network.name} for token ${TOKEN_ADDRESS} as ${wallet.address}...`,
  );

  const tx = await token.acceptOwnership();
